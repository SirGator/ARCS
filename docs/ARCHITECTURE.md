...
| Adapters | `src/adapters/` | Input (CLI), Interpretation (regelbasiert + API-Anbindung via Worker) |
...
| `IInputInterpreter` | `adapters/interpretation/include/interpretation/input_interpreter.hpp` | Rohtext → nicht-autoritative `interpretation_proposal` |
| `InterpretationToTaskMapper` | `adapters/interpretation/include/interpretation/interpretation_to_task_mapper.hpp` | Proposal → task draft, wenn sicher genug |
...
| Komponente | Status |
|------------|--------|
| `ApiInterpreter` | OpenAI-kompatibler Remote-Adapter für Interpretationsvorschläge |
| `RuleBasedInterpreter` | Implementiert — einfache Keyword-Interpretation |
| `InterpretationToTaskMapper` | Implementiert — task draft aus sicheren Vorschlägen |
